...
<style>
    ...
    <p id="gecenZaman" style="background: #fcba03; color: #3d03fc; font-weight: bold; font-size: 20px; border-radius: 5px; border: 3px solid white; padding: 5px;"></p>
    </div>

    <script>
        // Hesaplama fonksiyonu
        function gecenZamaniHesapla(hedefTarih) {
            const simdikiTarih = new Date();
            const hedef = new Date(hedefTarih);

            const gecenZaman = Math.floor((simdikiTarih - hedef) / 1000);

            if (gecenZaman < 60) {
                return gecenZaman + ' seconds ago';
            } else if (gecenZaman < 3600) {
                const dakika = Math.floor(gecenZaman / 60);
                return dakika + (dakika === 1 ? ' minute ago' : ' dakika önce');
            } else if (gecenZaman < 86400) {
                const saat = Math.floor(gecenZaman / 3600);
                return saat + (saat === 1 ? ' hours ago' : ' saat önce');
            } else {
                const gun = Math.floor(gecenZaman / 86400);
                return gun + (gun === 1 ? ' days ago' : ' gün önce');
            }
        }

        // Hedef tarih
        const hedefTarih = '<?= $row['cron_time'] ?>';

        // Geçen zamanı hesapla
        const gecenZamanMetni = gecenZamaniHesapla(hedefTarih);

        // Sonucu ekrana yazdır
        const sonuc = document.getElementById('gecenZaman');
        sonuc.innerHTML = `Cron manuel run: ${gecenZamanMetni}`;
    </script>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('backend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u718774278/domains/h5.tradervipai.com/public_html/resources/views/backend/edit/settings2.blade.php ENDPATH**/ ?>