...
class PtcController extends Controller
{
    public function index()
    {
        ...
        // ->limit(45)
        ->get();
        $totalMining = Transaction::where('user_id', auth()->user()->id)->where('remark', 'mining')->sum('amount');

        $todayMining = Transaction::where('user_id', auth()->user()->id)->where('remark', 'mining')->whereDate('created_at', Carbon::today())->sum('amount');

        return view(activeTemplate().'user.ptc.index',compact('ads', 'pageTitle', 'user', 'totalMining', 'todayMining'));
    }
    ...
    public function coinPrice(){
        $general = gs();
        $data = getCoinRate($general->coin_one, $general->coin_two, $general->cryptocompareApi);
        return response()->json(['cls'=>'success', 'data'=>$data],200);
    }

    //coin Price
    // public function coinPrice(){
    //     $general = gs();
    //     if ($general->coin_link) {
    //         $html = file_get_html($general->coin_link);
    //         $data = $html->find('div.hero-coin__price',0)->plaintext;

    //         // $html = file_get_html('https://www.coindesk.com/price/bitcoin/');
    //         // $data = $html->find('.currency-pricestyles__Price-sc-1rux8hj-0',0)->innertext;
    //     } else{
    //         $data= "Coin price not available";
    //     }
    //     return response()->json(['cls'=>'success', 'data'=>$data],200);
    // }
