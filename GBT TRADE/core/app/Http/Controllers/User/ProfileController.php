...
class ProfileController extends Controller
{
    ...
    public function withdrawWallet(){
        ...
        $user = Auth::user();
        return view($this->activeTemplate . 'user.withdraw.wallet_setup', compact('pageTitle', 'user'));
    }
    public function submitWithdrawWallet(Request $request){
        $request->validate([
            'withdraw_wallet'=> 'required'
        ]);

        $user = auth()->user();
        $user->withdraw_wallet = $request->withdraw_wallet;
        $user->save();
        ...
        $notify[] = ['success','Withdraw Wallet Set successfully!'];
        return back()->withNotify($notify);
