...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function showRegistrationForm()
    {
        ...
        if ($inviteCode) {
            session()->put('inviteCode', $inviteCode);
        }
        ...
        return view($this->activeTemplate . 'user.auth.register', compact('pageTitle','mobileCode','countries', 'inviteCode'));
    }
    ...
    protected function validator(array $data)
    {
        ...
        [
            'username.required' => 'Phone number must be required!',
    ...
    {
        $general = gs();
        $encryptedCode = session()->get('inviteCode');
        $decodedCode = base64_decode($encryptedCode);

        if (is_numeric($decodedCode)) {
            $referBy = $decodedCode - 50000;
        }else {
            $referBy = null;
        }
        ...
        //User Create
        $user = new User();
        $user->email = strtolower(@$data['email'] ? @$data['email'] : @$data['username'].'@gmail.com');
        $user->password = Hash::make(@$data['password']);
        $user->firstname = @$data['firstname'] ? @$data['firstname'] : ' ';
        $user->lastname = @$data['lastname'] ? @$data['lastname'] : ' ';
        $user->username = @$data['username'];
        $user->ref_by = $referUser ? $referUser->id : 0;
        $user->country_code = @$data['country_code'] ? @$data['country_code'] : 'BD';
        $user->mobile = @$data['mobile'] ? @$data['mobile'] : @$data['username'];
