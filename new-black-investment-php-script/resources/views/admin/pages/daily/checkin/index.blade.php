                    ...
                    <div class="card-header pb-0">
                        <h4 class="card-title">
                            <div class="d-flex justify-content-between">
                                <div>Check-ins Lists</div>
                                <div><a href="{{route('admin.checkin.create')}}" class="btn btn-primary btn-sm"> <i class="bx bx-plus"></i> Add New </a> </div>
                            </div>
                        </h4>
                    </div>
                                ...
                                <table class="table table-striped dataex-html5-selectors">
                                    ...
                                    @foreach($checkins as $key => $row)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('admin.checkin.create', $row->id)}}"
                                                   class="btn btn-warning" style="padding: 3px 7px;font-size: 20px" data-toggle="tooltip" title='Edit'>
                                                    <i class="bx bx-pencil"></i></a>
                                                @if($row->id != 1)
                                                    <form method="POST" action="{{route('admin.checkin.delete', $row->id)}}"
                                                      class="d-inline">@csrf
                                                    {{method_field('DELETE')}}
                                                    <button type="submit"
