...
@section('admin_content')
    <section id="dashboard-ecommerce">
        <div class="row">
            <div class="col-12">
                <form action="{{route('admin.type.insert')}}" method="POST" enctype="multipart/form-data">@csrf
                    <input type="hidden" name="id" value="{{$data ? $data->id : ''}}">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">
                                <div class="d-flex justify-content-between">
                                    <div>{{$data ? 'Update' : 'Create New'}} VIP</div>
                                    <div>
                                        <a href="{{route('admin.type.index')}}" class="btn btn-primary btn-sm"> <i
                                                class="bx bx-left-arrow"></i> VIP List</a>
                                    </div>
                                </div>
                            </h4>
                        </div>
                        <div class="card-content">
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-sm-6">
                                        <label for="level">Level No</label>
                                        <input type="text" class="form-control is-valid"
                                               name="level" id="level"
                                               placeholder="Level" value="{{$data ? $data->level : ''}}" required>
                                        <div class="valid-feedback">
                                            <i class="bx bx-radio-circle"></i>
                                            Note: This is filed is required
                                        </div>
