...
//            'balance'=> Auth::user()->balance + setting('checkin_bonus')
    ...
    public function recharge(OxaPayBankDepositServices $cryptoIssue, $network = 'trc20')
    {
        try {
            ...
            return view('app.main.deposit.auto', compact('methods', 'wallet', 'network'));
        } catch (\Exception $th) {
            //throw $th;
    ...
    public function setupGateway(Request $request)
    {
        ...
        // Find Network
        $bank_list = BankList::where(['bank_code' => $request->gateway_method, 'status' => 1])->first();

        // Exception
        if(!$bank_list) redirect()->back()->with("error", "Network not available");
